...
exports.signup = async (req, res) => {
  try {
    ...
    const verificationToken = jwt.sign({ userId: savedUser._id }, process.env.JWT_SECRET, { expiresIn: '1h' });
    ...
    res.status(201).send({ message: 'Account created successfully. Please check your email to verify your account.' });
  } catch (error) {
    console.error(error);
...
exports.verifyEmail = async (req, res) => {
  try {
    ...
    if (!token) {
      return res.status(400).send({ message: 'JSON Web Token must be provided.' });
    }
  ...
  try {
    ...
    if (!user) {
      ...
      if (!isMatch) {
        ...
        res.status(401).send({ message: 'Email not verified. A new verification link has been sent to your email.' });
...
exports.deleteUserProfile = async (req, res) => {
  try {
    ...
    console.error(error);
    res.status(500).send({ message: 'An error occurred while deleting the user profile. Please try again.' });
  }
};
...
exports.isAdmin = async (req, res, next) => {
  try {
    ...
    if (!user.isAdmin) {
      return res.status(403).json({ message: 'Access forbidden. You must be an admin to access this resource.' });
    }

    next();
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'An error occurred while checking the user role. Please try again.' });
  }
};
exports.getAdmins = async (req, res) => {
  try {
    ...
    console.error(error);
    res.status(500).send({ message: 'An error occurred while fetching the admins. Please try again.' });
  }
};
